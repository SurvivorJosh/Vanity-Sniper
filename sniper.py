    ...
    def change_van(self):
        ...
        try:
            if response.status_code == 200:
                logging.info(f" VANITY SNIPED : https://discord.gg/{van_code} is now yours")
                self.not_sniped = False
                os._exit(0)
            else:
                logging.info(f"COULDN'T SNIPE : for some odd reasoon couldn't snipe discord.gg/{van_code}")
    ...
    def sniper(self):
        ...
        try:
            if res.status_code == 404:
                logging.info(f" VANITY IS AVAILABLE : https://discord.gg/{van_code} was just dropped...sniping it")
                self.change_van()
